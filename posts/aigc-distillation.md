...
学生生成 xᵍ
  │
  ├─ 每隔 K 次更新 student Gθ
  │    冻结 Fφ、Dψ 和 real teacher
  │    L_DMD + λGAN L_GAN → θ
  │
  └─ 每次都更新 guidance
       ├─ fake teacher Fφ：对 sg(xᵍ) 加噪，回归 ε - xᵍ
       └─ discriminator Dψ：区分真实样本与 sg(xᵍ)
```
...
```text
student Gθ 从纯噪声生成动态少步轨迹
  │
  ├─ 每次更新 fake teacher Fφ
  │    从轨迹抽取 sg(x₀)，随机加噪
  │    L_fake = ‖Fφ(xσ, σ) - (ε - x₀)‖²
  │    optimizer_fake → φ
  │
  └─ 每隔 R 次更新 student Gθ
       冻结 real teacher 与 Fφ
       CA + on-trajectory DM + off-trajectory DM
       optimizer_student → θ
```
