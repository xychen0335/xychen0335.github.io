...
* adaLN-zero block：
    * adaLN：原始层归一化为$y = \frac{x - \mu}{\sigma} \cdot \gamma + \beta$。通常 $\gamma$ (scale) 和 $\beta$ (shift) 是固定的、可学习的参数。自适应层归一化中$\gamma$ 和 $\beta$ 不是固定的，而是根据条件 ($t$ 和 $c$) 实时算出来的。网络会把时间 $t$ 和类别 $c$ 相加后输入一个 MLP（多层感知机），这个 MLP 输出 6 个参数，用来动态控制 Transformer Block 里的 Layer Norm 和残差连接的缩放。
    * zero：最后输出的线性层权重初始化为 0，类似于 ResNet ，这使得网络在初始时为恒等映射。这稳定了深层网络的训练，让模型更容易收敛。
* Incontext-conditioning：类似于 GPT，将时间和条件作为额外的 token 添加到输入序列。
* Cross-attention：将时间和条件的嵌入连接到一个长度为 2 的序列，与图像 token 序列分开。在多头自注意力模块后添加一个额外的多头交叉注意力模块，图像 token （Q）去查询条件序列（K, V）。
...
- Flux：
  - 文本编码器使用多模态模型。
  - 扩散骨干采用双流 MM-DiT 和单流 MM-DiT 的混合架构。
  - 位置编码采用 3D RoPE，对于编辑任务，可以实现输入与参考图的区分。
  - 对于编辑任务，输入与参考图在被投影为 token 后，在 token 序列层面进行拼接。
- Qwen Image：
  - 文本编码器使用 Qwen-VL，扩散骨干采用原始的双流 MM-DiT，规模大 20B。
  - 在进行编辑时，参考图像 → Qwen2.5-VL → 语义表示，参考图像 → VAE Encoder → 外观/重建表示。
  - 位置编码采用 MSRoPE（多模态可扩展旋转位置编码）。
- Z-Image：
  - 文本编码器采用 Qwen，扩散骨干采用纯单流 MM-DiT，轻量 6B。
  - 对于编辑任务，额外采用 SigLIP2 获取高层视觉语义。
  - 使用 3D Unified RoPE。编辑任务中，参考图像标记和目标图像标记被分配对齐的空间 RoPE 坐标，但在时间维度中通过单位间隔分隔。此外，参考图像和目标图像会应用不同的时间条件值，以区分干净和噪声图像。
  - 采用 DMDR 进行蒸馏。
- HunYuan：
  - 80B MoE 自回归原生多模态，统一多模态理解和生成。
- SenseNova-U1：
  - 抛弃了 AE 和 VAE，对于视觉模态，采用两层卷积 + MLP 进行嵌入。
  - 原生统一多模态理解与生成。 
