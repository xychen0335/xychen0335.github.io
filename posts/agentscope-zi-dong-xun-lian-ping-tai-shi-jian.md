...
- **昂贵**：每一步都可能牵涉申请 GPU、启动训练实例、投递命令、释放资源，动作错了就是真金白银。
...
这里有个必须提醒的坑：快照里藏着**可重建的运行时缓存**。我们踩过一次事故——一个 70KB 的二进制 PDF 被文件工具按行解码塞进缓存，膨胀成约 37MB，JSON 序列化后单条快照接近 100MB。修复思路值得复用：**落盘前剥离一切「可重建、非权威」的缓存，只保留真正影响模型行为的状态**（上下文、summary、已激活工具组）。恢复后工具会按需重新读取文件，拿到的反而是最新内容。

快照恢复时还要做「治愈」：删除不成对的 tool_call/tool_result block、把空参数归一成 `{}`，否则一个卡死的会话会一直卡下去。用户中断后保存的快照，还要把「正在等待审批/执行的工具」补成 interrupted 结果，避免下一轮 AgentScope 以为还在等工具返回。
...
早期很容易犯的错是把技能全量塞进 system prompt。技能一多，上下文直接爆炸，而且大多数技能当前任务根本用不上。我们最终的做法是：**技能以「目录 + SKILL.md」的形式存在，正文不进 prompt，Agent 通过内置的 SkillViewer 工具按需检索**。
...
每次复现完成，平台可以做一次「知识提取」：把论文/仓库的事实（facts）和固定 commit 的代码（pinned worktree）作为输入，蒸馏出一个可复用的 Skill，包含 SKILL.md、核心想法、可调整参数和**真实代码锚点**。
...
写回也有边界：经验只写回技能的**参考层**（`references/repair/*.md`），用来指导「怎么写 wrapper、怎么排查」，永远不碰状态机、控制器、评分逻辑这些**裁判**。进化不能修改裁判，否则整个闭环就失去了客观性。
...
2. **Agent 框架当库用**。库式接入意味着事件、存储、审批这些业务边界在自己手里，框架负责 Agent 装配、推理与工具执行，替换成本最低。
